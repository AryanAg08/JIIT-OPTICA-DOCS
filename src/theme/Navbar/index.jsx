    ...
    return (
        <span className="navbar">
            <div className={`relative ${isMenuOpen ? "overflow-hidden" : ""}`}>
                        ...
                        <div className="hidden md:flex space-x-8 text-lg font-medium">
                            {navbarItems.map((item) => (
                                <Link
                                    key={item.label}
                                    to={item.to}
                                    className="cursor-pointer relative group text-gray-300 transition-all duration-300"
                                >
                                    <span className="group-hover:font-semibold transition-all">{item.label}</span>
                                    {/* Minimalist White Underline Effect */}
                                    <span className="absolute bottom-[-3px] left-0 w-0 h-[2px] bg-white transition-all duration-500 group-hover:w-full"></span>
                                </Link>
                            ))}
                        </div>
                    </div>
                ...
                <div
                    className={`fixed top-0 right-0 w-2/4 md:w-1/2 h-full bg-gray-800 bg-opacity-95 transform transition-all duration-300 ease-in-out ${isMenuOpen ? "translate-x-0" : "translate-x-full"
                        } z-40`}
                >
